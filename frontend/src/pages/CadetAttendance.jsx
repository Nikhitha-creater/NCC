...
export default function CadetAttendance() {
  ...
  useEffect(() => {
    api.cadets.attendance(user?.id || "me")
      .then(res => setRecords(res?.records || res || MOCK))
      ...
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div className="card">
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Date</th><th>Event Type</th><th>Status</th><th>Remarks</th></tr>
              </thead>
              <tbody>
                {filtered.length === 0
                  ? <tr><td colSpan={4}><EmptyState icon="📋" title="No records" /></td></tr>
                  : filtered.map(r => (
                    <tr key={r.id} className={`att-row-${r.present?"present":"absent"}`}>
                      ...
                      <td>{r.type}</td>
                      <td><Badge variant={r.present?"success":"danger"}>{r.present?"✓ Present":"✕ Absent"}</Badge></td>
