        ...
        <header className="topbar">
          ...
          <div className="topbar-right">
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--navy-900)" }}>{user?.name}</div>
              <div style={{ fontSize: 11, color: "var(--slate-500)", textTransform: "capitalize" }}>
                {user?.unit || "NCC Unit"} · {user?.role}
              </div>
            </div>
            <div className="sidebar-avatar" style={{ border: "2px solid var(--navy-200)", color: "var(--navy-700)", background: "var(--navy-50)" }}>
              {initials}
            </div>
          </div>
        </header>
